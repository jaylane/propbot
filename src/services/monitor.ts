import type { Client, TextChannel } from 'discord.js';
import {
  getAllActiveMonitors, getActiveSlips, getLegs, touchMonitor, setMonitorActive,
  type MonitorRow,
} from '../db/index.js';
import { evaluateSlip } from './prop-tracker.js';
import { buildStatusEmbed, buildSettledEmbed } from '../utils/embeds.js';

// Per-guild interval handles
const timers = new Map<string, NodeJS.Timeout>();

export function startAllMonitors(client: Client): void {
  const monitors = getAllActiveMonitors();
  for (const monitor of monitors) {
    scheduleMonitor(client, monitor);
  }
  console.log(`[Monitor] Started ${monitors.length} active monitors`);
}

export function startMonitor(client: Client, monitor: MonitorRow): void {
  stopMonitor(monitor.guild_id);
  scheduleMonitor(client, monitor);
  console.log(`[Monitor] Started for guild ${monitor.guild_id} every ${monitor.interval_ms / 1000}s`);
}

export function stopMonitor(guildId: string): void {
  const timer = timers.get(guildId);
  if (timer) {
    clearInterval(timer);
    timers.delete(guildId);
  }
  setMonitorActive(guildId, false);
}

function scheduleMonitor(client: Client, monitor: MonitorRow): void {
  const timer = setInterval(() => runMonitorCycle(client, monitor.guild_id), monitor.interval_ms);
  timers.set(monitor.guild_id, timer);

  // Run immediately on first start
  runMonitorCycle(client, monitor.guild_id).catch(console.error);
}

async function runMonitorCycle(client: Client, guildId: string): Promise<void> {
  const monitors = getAllActiveMonitors();
  const monitor = monitors.find(m => m.guild_id === guildId);
  if (!monitor) return;

  const activeSlips = getActiveSlips(guildId);
  if (!activeSlips.length) return;

  let channel: TextChannel | null = null;
  try {
    const ch = await client.channels.fetch(monitor.channel_id);
    if (ch?.isTextBased()) channel = ch as TextChannel;
  } catch {
    console.warn(`[Monitor] Cannot access channel ${monitor.channel_id} for guild ${guildId}`);
    return;
  }

  touchMonitor(guildId);

  const settled: number[] = [];

  for (const slip of activeSlips) {
    try {
      const prevStatus = slip.status;
      const evals = await evaluateSlip(slip.id);

      // Reload slip after evaluation to check if it settled
      const { getSlip } = await import('../db/index.js');
      const updatedSlip = getSlip(slip.id);

      if (updatedSlip && updatedSlip.status !== 'active' && prevStatus === 'active') {
        // Slip just settled
        settled.push(slip.id);
        if (channel) {
          const legs = getLegs(slip.id);
          await channel.send({ embeds: [buildSettledEmbed(updatedSlip, legs)] });
        }
      } else if (evals.length > 0 && channel) {
        // Post live update
        const legs = getLegs(slip.id);
        const embed = buildStatusEmbed(slip, legs, evals);
        if (embed) await channel.send({ embeds: [embed] });
      }
    } catch (err) {
      console.error(`[Monitor] Error evaluating slip ${slip.id}:`, err);
    }
  }
}
